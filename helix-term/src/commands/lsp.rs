...
use helix_view::{document::Mode, editor::Action, theme::Style, DocumentId, ViewId};
...
fn sym_picker(
    option_manager: OptionsManager<SymbolInformationItem>,
    ...
    FilePicker::new(
        ...
        move |cx,
              ...
              action| {
            // TODO use current view if other view was closed already?
            if !cx.editor.tree.contains(view_id) || cx.editor.document(doc_id).is_none() {
                return cx.editor.set_error("view or document is closed already");
            }
            let (view, doc) = (view_mut!(cx.editor, view_id), doc!(cx.editor, &doc_id));
            push_jump(view, doc);
            ...
            let (view, doc) = (view_mut!(cx.editor, view_id), doc_mut!(cx.editor, &doc_id));
...
pub fn symbol_picker(cx: &mut Context) {
    ...
    let create_symbols_item_source = |language_server: &&helix_lsp::Client| {
        let current_url = doc.url();
        let doc_identifier = doc.identifier();
        let offset_encoding = language_server.offset_encoding();

        let symbols_request = language_server.document_symbols(doc.identifier())?;
        let fetch_items_future = async move {
            let json = symbols_request.await?;
            ...
            let symbols = match symbols {
                ...
                lsp::DocumentSymbolResponse::Nested(symbols) => {
                    ...
                    for symbol in symbols {
                        nested_to_flat(&mut flat_symbols, &doc_identifier, symbol, offset_encoding)
                    }
                    flat_symbols
                }
            };
        ...
        .boxed();
        Some(ItemSource::from_async_data(
            fetch_items_future,
            current_url.clone(),
        ))
    };
    ...
    let item_sources = doc
        ...
        .iter()
        .filter_map(create_symbols_item_source)
        .collect();
    ...
    OptionsManager::create_from_item_sources(
        ...
        move |_editor, compositor, option_manager| {
            let picker = sym_picker(option_manager, current_url, doc_id, view_id);
            compositor.push(Box::new(overlayed(picker)))
        },
        Some(Box::new(|editor| {
            editor.set_error("No Language server for this document does support document symbols");
        })),
    );
}
...
pub fn workspace_symbol_picker(cx: &mut Context) {
    ...
    let create_workspace_symbols_fetcher = |language_server: &helix_lsp::Client| {
        let language_server_id = language_server.id();
        let offset_encoding = language_server.offset_encoding();

        let fetch_items = move |pattern: &str, editor: &mut Editor| {
    ...
    let current_url = doc.url();
    let item_sources = language_servers
        .iter()
        .map(|language_server| {
            ItemSource::from_async_refetch_on_idle_timeout_with_pattern(
                create_workspace_symbols_fetcher(language_server),
                current_url.clone(),
            )
        })
        .collect();
...
pub fn code_action(cx: &mut Context) {
    ...
    let mut item_sources = Vec::with_capacity(language_servers.len());
    // let create_code_action_item_source = move |language_server: &&helix_lsp::Client| {
    for language_server in language_servers {
        let offset_encoding = language_server.offset_encoding();
        let language_server_id = language_server.id();
        let range = range_to_lsp_range(&text, selection_range, offset_encoding);
        let future = match language_server.code_actions(
            ...
            Some(future) => future,
            None => {
                // TODO maybe outsource this into the "NoItemsAvailableCallback" in the OptionsManager, as error source,
                // as other language server which support code actions (but don't have any) would overwrite this message below with
                // "No code actions available"
                // apart from the borrow-checking issues this currently provides
                // cx.editor
        ...
        .boxed();
        item_sources.push(ItemSource::from_async_data(item_source, ()));
    }
    ...
    OptionsManager::create_from_item_sources(
        ...
        move |_editor, compositor, options_manager| {
            let mut picker = Menu::new(options_manager, move |editor, action, event| {
        ...
        Some(Box::new(|editor| {
            editor.set_status("No Language server does support code actions or there are no code actions available")
        })),
    );
}
