...
pub fn trigger_auto_completion(
    ...
    if config.auto_completion {
        ...
        let trigger_path_completion = rope_ends_with("/", text) && doc.supports_path_completion();

        if is_trigger_char || is_auto_trigger || trigger_path_completion {
            send_blocking(
                tx,
                CompletionEvent::Trigger(CompletionTrigger {
                    ...
                    doc: doc.id(),
                    view: view.id,
                    auto: !is_trigger_char && !trigger_path_completion,
                }),
            );
        }
    }
